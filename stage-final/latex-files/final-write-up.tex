...
% Title Page
\title{F1 Database}
\author{Kriza del Moro, Ayesha Qadir, Jatinder Sahota}
...
Our group chose to model all data from past seasons of Formula 1. There's a lot of potential for analysis, and a well-designed data model will best allow for that. We included as much data as possible to allow for full analysis (or for full access to data for a layperson). Our UI is a lightweight text-based interface. It categorizes all available queries to make the data easily accessible, and takes into account those who are less experienced with CLIs by including a help menu. This report details the process of creating this project, and further explains its features.
...
The data consists of tables pertaining to the following: races, drivers, constructors, qualifying sessions, circuits, lap times, and the championships overall.
...
Our dataset required quite a bit of cleaning, largely in the form of cutting columns we felt weren't necessary. Many were cut due to having excessive null values, though some (like information on pit stops) were cut to keep the scope of our project more reasonable. In the remaining data, went through and changed null values to more appropriate values (where null represented some status or other information).

We also added on table on our own, made from scratch. Locale was created by hand to map nationalities to their countries.

Below is the final version of the ER diagram that we created based off of the dataset.

\begin{figure} [h!]
    \centering
    \includegraphics[width=0.8\linewidth]{ER-diagram.png}
    \caption{ER diagram for our dataset}
...
As mentioned earlier, the bulk of the data comes from laps. This made populating the database somewhat difficult because it took up quite a bit of memory to do so. Transactions were not viable because MS SQL was causing issues regarding duplicate entries that SQLite didn't. In the end, we had to populate the data on laps a little bit at a time (about 50,000 entries at a time) to make it work.
...
See Figure 2 below for some screenshots of the UI in action. 
...
\begin{figure}[h!]
  ...
  \end{subfigure}
  \begin{subfigure}[b]{0.7\linewidth}
    \includegraphics[width=\linewidth]{help-menu.png}
    \caption{Help menu}
  \end{subfigure}
  \begin{subfigure}[b]{0.4\linewidth}
    \includegraphics[width=\linewidth]{promptoutput.png}
    \caption{Driver information and query output}
  \end{subfigure}
  \caption{Screenshots of the UI}
  \label{fig:coffee3}
\end{figure}
...
\subsection{Which driver contributed the most to a specific constructor in a season based on points earned}
...
\subsection{Which driver contributed the most to a specific constructor in a season based on races completed}
Similar to above, but the metric is now the amount of races the driver had completed.

Same as previous.

\subsection{Which nationality drove the fastest average lap on circuits native to their country}
...
\subsection{Average lap time over the entirety of a given season}
...
\subsection{Which race in a give season had the most drivers who did not finish (DNF)}
...
\subsection{Stage 1}
\begin{itemize}
    \item All 3 members helped look for and decide on a dataset
    \item Ayesha wrote the project timeline
    \item Jatinder wrote the summary of dataset, created a Git repository for the project, and submitted all finished work
\end{itemize}

\subsection{Stage 2}
\begin{itemize}
    \item All 3 members discussed and decided on how to separate the dataset into entities and relationships
    \item KC wrote the summary paragraph for the dataset
    \item Ayesha drew the ER diagram, and submitted all finished work
\end{itemize}

\subsection{Stage 3}
\begin{itemize}
    \item All 3 members dicussed and decided how to modify the ER diagram based on feedback from earlier stages, and made a bullet-point draft of the group reflection together
    \item Ayesha modified the Stage 1 summary of the dataset as needed and updated the timeline
    \item Jatinder converted the ER diagram to the relational model, including merging and normalizing
    \item KC wrote the group reflection based on the group's bullet-point notes, and submitted all finished work
\end{itemize}

\subsection{Stage 4}
\begin{itemize}
    \item All 3 members brainstormed and decided on queries together, and discussed how to implement feedback from earlier stages
    \item Jatinder submitted the finalized list of queries
\end{itemize}

\subsection{Stage 5}
\begin{itemize}
    \item All 3 members discussed how to implement Stage 4 feedback, and implemented approximately 1/3 of the group's queries along with writing descriptions for them
    \item Ayesha submitted final copies of everything (though due to some issues with Git, one member's work did not get submitted)
\end{itemize}

\subsection{Stage 6}
\begin{itemize}
    \item All 3 members discussed and decided on UI design, and made a bullet-point draft of the group reflection together
    \item Ayesha wrote out the group reflection and the UI description, and created the diagrams for the UI
    \item Jatinder updated the group timeline 
    \item KC cleaned all the data in preparation for the final stage, and submitted all finished work
\end{itemize}

\subsection{Final Stage}
\begin{itemize}
    \item All 3 members continually discussed and decided on minor changes and implementation details, made a bullet-point draft of the group reflection together, and rehearsed for the final demonstration together
    \item Ayesha wrote the final write-up and final reflection based on the group's bullet-point notes
    \item KC implemented the frontend/UI in Java, including creating scaffolding for the backend to be connected
    \item Jatinder set up the database on Uranium, implemented all queries to work on Uranium and connect to the frontend, connected the front- and backend, and submitted all finished work
\end{itemize}
