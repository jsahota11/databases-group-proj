...
// // printing pretty
// // perhaps some input santizing if bad params are given (out of bounds IDs
// for example)
//
...
// if (!validIn) {
// System.out.println("Please enter a valid option. Type h to print the help
// menu.");
// }
// }
//
...
// db.driverContributionsPoints(Integer.parseInt(args[0]),
// Integer.parseInt(args[1]));
// } else if (input.equals("dcr") && args.length == 2) {
// db.driverContributionsRaces(Integer.parseInt(args[0]),
// Integer.parseInt(args[1]));
// } else if (input.equals("dp") && args.length == 3) {
// db.positionInstance(Integer.parseInt(args[0]), Integer.parseInt(args[1]),
...
// db.driversFastestSeasonLap(Integer.parseInt(args[0]),
// Integer.parseInt(args[1]));
...
//
// System.out.println(
// "<line number> <parameter> - See the corresponding query in the .md file, and
// provide a parameter if necessary.");
...
// String sql = "select d.driverId, d.firstname, d.lastname, sum(dr.points) as
// totalPoints from driverRaces dr join races r on r.raceId = dr.raceId join
// drivers d on d.driverId = dr.driverId where year = ? group by d.driverId,
// d.firstname, d.lastname order by totalPoints desc";
// try {
//
...
// + resultSet.getString("lastname") + " | Total Points: " +
...
// String sql = "with seasonCounts as ( select c.name, count(r.raceId) as
// raceCount from races r join circuits c on r.circuitId = c.circuitId where
// r.year = ? group by c.name) select * from seasonCounts order by raceCount
// desc;";
//
// try {
//
...
// }
//
// // Find some circuit with the highest number of finished races for each
// season
// public void mostFinishedRaces() {
// String sql = "with circuitCounts as ( select r.year, c.circuitId, c.name,
// c.country, c.location, count(r.raceId) as raceCount from races r join
// circuits c on r.circuitId = c.circuitId group by r.year, c.circuitId, c.name,
// c.country, c.location) select cc.* from circuitCounts cc join (select year,
// max(raceCount) as maxCount from circuitCounts group by year) x on cc.year =
// x.year order by cc.year;";
//
// try {
//
...
// // String sql = "select r.name, count(dr.driverId) as dnfCount from races r
// join
// // driverRaces dr on r.raceId = dr.raceId where r.raceId = ? and
// // dr.finalPosition = -1 group by r.raceId, r.name;";
// //
...
// String sql = "select r.year, avg(dr.gridPosition) as avgPos from driverRaces
// dr join drivers d on dr.driverId = d.driverId join races r on r.raceId =
// dr.raceId where d.driverId = ? group by r.year order by r.year;";
//
// try {
//
// PreparedStatement statement = connection.prepareStatement(sql);
...
// // figure out how to make this work, dont want to use two separate queries
// but
// // might have to
...
// String sql = "select top (1) d.driverId, d.firstname, d.lastname, d.dob from
// drivers d join partOf p on d.driverId = p.driverId where p.year = ? order by
// d.dob union select top (1) d.driverId, d.firstname, d.lastname, d.dob from
// drivers d join partOf p on d.driverId = p.driverId where p.year = ? order by
// d.dob desc;";
//
// try {
...
// }
//
// // given a season and constructor, get the driver that contributed the most
// to a
// // constructor by points earned
// public void driverContributionsPoints(int year, int constructorId) {
// String sql = "select top (1) d.driverId, d.firstname, d.lastname,
// sum(dr.points) as totalPoints from driverRaces dr join races r on dr.raceId =
// r.raceId join drivers d on d.driverId = dr.driverId join partOf p on
// p.driverId = dr.driverId and p.year = r.year where r.year = ? and
// p.constructorId = ? group by d.driverId, d.firstname, d.lastname order by
// totalPoints desc;";
//
// try {
//
...
// String sql = "select top (1) d.driverId, d.firstname, d.lastname,
// count(dr.raceId) as raceCount from driverRaces dr join races r on dr.raceId =
// r.raceId join drivers d on d.driverId = dr.driverId join partOf p on
// p.driverId = dr.driverId and p.year = r.year where r.year = ? and
// p.constructorId = ? group by d.driverId, d.firstname, d.lastname order by
// raceCount desc;";
...
// // seasons s join races r on r.year = s.year join laps l on l.raceId =
// r.raceId
...
// }
//
// // which nationality drove the fastest average lap on circuits native to
// their
// // country
...
// String sql = "select position from laps where driverId = ? and raceId = ? and
// lapNumber = ?;";
//
// try {
//
...
// String sql = "select top (1) d.driverId, d.firstname, d.lastname,
// sum(dr.points) as totalPts from driverRaces dr join races r on r.raceId =
// dr.raceId join drivers d on d.driverId = dr.driverId where r.year = ? group
// by d.driverId, d.firstname, d.lastname order by totalPts desc;";
//
// try {
//
// PreparedStatement statement = connection.prepareStatement(sql);
...
// while (resultSet.next()) {
// System.out.println(
// resultSet.getString("name") + ": Latitude = " + resultSet.getFloat("lat") +
// ", Longitude = "
...
// String sql = "select d.driverId, d.firstname, d.lastname from driverRaces dr
// join drivers d on d.driverId = dr.driverId where dr.raceId = ? and
// dr.finalPosition = 1;";
//
// try {
//
...
// String sql = "select d.driverId, d.firstname, d.lastname, dr.points from
// driverRaces dr join drivers d on d.driverId = dr.driverId where dr.raceId = ?
// and dr.driverId = ?;";
//
// try {
//
...
// System.out.println(
// resultSet.getInt("driverId") + ": " + resultSet.getString("firstname") + " "
// + resultSet.getString("lastname") + ", " + resultSet.getInt("points") + "
// points.");
// }
// }
//
...
// String sql = "select c.constructorId, c.name, cr.points from constructorRaces
// cr join constructors c on c.constructorId = cr.constructorId where cr.raceId
// = ? and cr.constructorId = ?;";
//
// try {
//
// PreparedStatement statement = connection.prepareStatement(sql);
...
// String sql = "select d.driverId, d.firstname, d.lastname, count(distinct
// r.year) as seasonCount from driverRaces dr join drivers d on d.driverId =
// dr.driverId join races r on r.raceId = dr.raceId group by d.driverId,
...
// String sql = "select c.constructorId, c.name, count(distinct r.year) as
// seasonCount from constructorRaces cr join constructors c on c.constructorId =
// cr.constructorId join races r on r.raceId = cr.raceId group by
...
// // dr.qual1 as qualTime from driverRaces dr join drivers d on d.driverId =
// // dr.driverId where dr.qual1 is not null union select top (1) d.driverId,
// // d.firstname, d.lastname, dr.raceId, dr.qual2 as qualTime from driverRaces
// dr
// // join drivers d on d.driverId = dr.driverId where dr.qual2 is not null
// union
// // select top (1) d.driverId, d.firstname, d.lastname, dr.raceId, dr.qual3 as
// // qualTime from driverRaces dr join drivers d on d.driverId = dr.driverId
// where
// // dr.qual3 is not null order by qualTime;";
// //
// // try {
...
// String sql = "select c.name, p.year from partOf p join drivers d on
// p.driverId = d.driverId join constructors c on c.constructorId =
// p.constructorId where p.driverId = ? ";
//
// try {
//
// PreparedStatement statement = connection.prepareStatement(sql);
...
// String sql = "select top (1) d.driverId, d.firstname, d.lastname, r.raceId,
// r.name, l.lapNumber, l.position, l.time from laps l join races r on l.raceId
// = r.raceId join drivers d on d.driverId = l.driverId where r.raceId = ? order
// by l.time;";
//
// try {
//
...
// String sql = "select top (1) l.* from laps l join races r on r.raceId =
// l.raceId where l.driverId = ? and r.year = ? order by l.time;";
// try {
//
// PreparedStatement statement = connection.prepareStatement(sql);
//
...
// + resultSet.getInt("driverId") + ", Lap Number: " +
// resultSet.getInt("lapNumber")
// + ", Position: " + resultSet.getInt("position") + ", Time: " +
...
// String sql = "select top (1) l.* from laps l join races r on l.raceId =
// r.raceId where r.year = ? order by l.time;";
// try {
//
// PreparedStatement statement = connection.prepareStatement(sql);
//
...
// String sql = "select c.* from circuits c where c.circuitId in (select top (1)
// r.circuitId as uses from races r group by r.circuitId order by
// count(r.raceId) desc);";
//
// try {
//
...
// ", Latitude = "
// + resultSet.getFloat("lat") + ", Longitude = " + resultSet.getFloat("lng") +
// ", Altitude = "
...
// String sql = "select top (1) r.raceId, r.name, count(dr.finalPosition) as fps
// from races r join driverRaces dr on dr.raceId = r.raceId where
// dr.finalPosition = -1 and r.year = ? group by r.raceId, r.name order by fps
// desc;";
// try {
//
// PreparedStatement statement = connection.prepareStatement(sql);
...
//
// while (resultSet.next()) {
// System.out.println(resultSet.getInt("raceId") + ": " +
// resultSet.getString("name") + ", "
...
// String sql = "select count(dc.driverId) as wins from driverChampionships dc
// where dc.driverId = ? and dc.winner = 1;";
// try {
//
// PreparedStatement statement = connection.prepareStatement(sql);
//
...
// String sql = "with driverCareers as (select d.driverId, count(p.year) as
// activeYears from drivers d join partOf p on d.driverId = p.driverId group by
// d.driverId), maxDriverCareers as (select driverId, activeYears from
// driverCareers where activeYears = (select max(activeYears) from
// driverCareers)) select d.*, m.activeYears from drivers d join
// maxDriverCareers m on m.driverId = d.driverId;";
// try {
//
// Statement statement = connection.createStatement();
//
...
//
// while (resultSet.next()) {
// System.out.println(resultSet.getInt("constructorId") + ": " +
// resultSet.getString("name") + ", "
...
//
// while (resultSet.next()) {
// System.out.println(resultSet.getInt("raceId") + ": Round " +
// resultSet.getInt("round") + ", Date: "
// + resultSet.getString("date") + ", Name: " + resultSet.getString("name") + ",
// Circuit ID: "
...
// while (resultSet.next()) {
// System.out.println(
// resultSet.getInt("circuitId") + " - Name: " + resultSet.getString("name") +
// ", Location: "
// + resultSet.getString("location") + ", " + resultSet.getString("country") +
// ", ("
...
//
// while (resultSet.next()) {
// System.out.println(resultSet.getInt("driverId") + " - Name: " +
// resultSet.getString("firstname") + " "
