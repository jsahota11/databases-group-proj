...
public class Database {
	...
	public void driverContributionsPoints(int year, int constructorId) {
		String sql = "select top (1) d.driverId, d.firstname, d.lastname, sum(dr.points) as totalPoints, r.year, c.name from driverRaces dr join races r on dr.raceId = r.raceId join drivers d on d.driverId = dr.driverId join partOf p on p.driverId = dr.driverId and p.year = r.year join constructors c on c.constructorId = p.constructorId where r.year = ? and p.constructorId = ? group by c.name, r.year, d.driverId, d.firstname, d.lastname order by totalPoints desc;";
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-14s %-27s %-18s %-25s %-6s%n", "Driver ID", "Driver Name", "Total Points",
				...
				while (resultSet.next()) {
					System.out.printf(
							"%-14d %-27s %-18d %-25s %-6d%n",
							resultSet.getInt("driverId"),
							resultSet.getString("firstname") + " " + resultSet.getString("lastname"),
							resultSet.getInt("totalPoints"), resultSet.getString("name"), resultSet.getInt("year"));
				}
			}
			System.out.println();
	...
	public void driverContributionsRaces(int year, int constructorId) {
		String sql = "select top (1) d.driverId, d.firstname, d.lastname, count(dr.raceId) as raceCount, r.year, c.name from driverRaces dr join races r on dr.raceId = r.raceId join drivers d on d.driverId = dr.driverId join partOf p on p.driverId = dr.driverId and p.year = r.year join constructors c on p.constructorId = c.constructorId where r.year = ? and p.constructorId = ? group by d.driverId, d.firstname, d.lastname, r.year, c.name order by raceCount desc;";
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-14s %-27s %-18s %-30s %-6s%n", "Driver ID", "Driver Name", "Races",
						"Constructor Name", "Year");
				System.out.println(
						"----------------------------------------------------------------------------------------------------------------");
				...
				while (resultSet.next()) {
					System.out.printf(
							"%-14d %-27s %-18d %-30s %-6d%n",
							resultSet.getInt("driverId"),
							resultSet.getString("firstname") + " " + resultSet.getString("lastname"),
							resultSet.getInt("raceCount"), resultSet.getString("name"), resultSet.getInt("year"));
				}
			}
	...
	public void quickestConstructor(int year) {
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-7s %-25s %-15s%n", "Year", "Name", "Total Time");
				System.out.println("-----------------------------------------------");
	...
	public void fastestOnNativeCircuits() {
		String sql = "with nativeLaps as ( select d.driverId, d.nationality, loc.country, l.time from laps l join drivers d on d.driverId = l.driverId join races r on r.raceId = l.raceId join circuits c on c.circuitId = r.circuitId join locale loc on d.nationality = loc.nationality and c.country = loc.country) select top (10) nationality, country, count(distinct driverId) as driverCount, dateadd(millisecond, avg(datediff(millisecond, 0, time)), 0) as avgTime from nativeLaps group by nationality, country order by avgTime;";
		...
		try {
			...
			System.out.printf("%-15s %-15s %-20s %-20s%n", "Nationality", "Avg Lap Time", "Number of Drivers",
					"Country");
			System.out.println("----------------------------------------------------------------");
	...
	public void positionInstance(int driverId, int raceId, int number) {
		String sql = "select d.*, r.*, l.* from laps l join races r on r.raceId = l.raceId join drivers d on d.driverId = l.driverId where d.driverId = ? and r.raceId = ? and l.lapNumber = ?;";
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-25s %-30s %-15s %-15s%n", "Driver Name", "Race Name", "Position", "Lap Number");
				System.out.println(
						"-------------------------------------------------------------------------------------");

				while (resultSet.next()) {
					System.out.printf("%-25s %-30s %-15d %-15d%n",
							resultSet.getString("firstname") + " " + resultSet.getString("lastname"),
							resultSet.getString("name"), resultSet.getInt("position"), resultSet.getInt("lapNumber"));
				}
			}
	...
	public void circuitCoords(int circuitId) {
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-35s %-12s %-12s %-15s%n", "Name", "Latitude", "Longitude", "Altitude");
				System.out.println("--------------------------------------------------------------------------");

				while (resultSet.next()) {
					System.out.printf(
							"%-35s %-12.3f %-12.3f %-15.3f%n",
							resultSet.getString("name"),
	...
	public void roundPtsDriver(int raceId, int driverId) {
		String sql = "select r.*, d.driverId, d.firstname, d.lastname, dr.points from driverRaces dr join drivers d on d.driverId = dr.driverId join races r on dr.raceId = r.raceId where dr.raceId = ? and dr.driverId = ?;";
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-19s %-30s %-10s %-25s %-10s%n", "Driver ID", "Driver Name", "Race ID", "Race Name",
						"Points");
				System.out.println(
						"------------------------------------------------------------------------------------------------");
				...
				while (resultSet.next()) {
					System.out.printf(
							"%-19d %-30s %-10d %-25s %-10d%n",
							resultSet.getInt("driverId"),
							resultSet.getString("firstname") + " " + resultSet.getString("lastname"),
							resultSet.getInt("raceId"), resultSet.getString("name"),
	...
	public void roundPtsConstructor(int raceId, int constructorId) {
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) { // empty?
				...
				System.out.printf("%-19s %-30s %-15s%n", "Constructor ID", "Name", "Points");
				System.out.println("-----------------------------------------------------------");
	...
	public void constructorChampionshipCount(int constructorId) {
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-20s %-15s%n", "Name", "Championships");
				System.out.println("----------------------------------------");
				...
				PreparedStatement secondStatement = connection.prepareStatement(
						"select distinct r.year as raceYear from races r join constructorRaces cr on r.raceId = cr.raceId where cr.constructorId = ?");

				secondStatement.setInt(1, constructorId);

				ResultSet resultSet2 = secondStatement.executeQuery();
				System.out.println("Season");
				System.out.println("----------");
				while (resultSet2.next()) {
					System.out.println(resultSet2.getInt("raceYear"));
				}
			}
	...
	public void driversConstructorList(int driverId) {
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-25s %-6s%n", "Name", "Year");
				System.out.println("-------------------------------");
	...
	public void fastestLapInRace(int raceId) {
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-12s %-25s %-12s %-12s %-10s %-12s %-25s%n",
	...
	public void mostUsedCircuit() {
		...
		try {
			...
			while (resultSet.next()) {
				System.out.printf(
						"%-15d %-30s %-15s %-15s %-12.3f %-12.3f %-10.3f%n",
						resultSet.getInt("circuitId"),
						resultSet.getString("name"),
						resultSet.getString("location"),
						resultSet.getString("country"),
	...
	public void driversNationality(int driverId) {
		String sql = "select * from drivers d where d.driverId = ?;";
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-12s %-25s %-15s%n", "Driver ID", "Name", "Nationality");
				System.out.println("----------------------------------------------------");

				while (resultSet.next()) {
					System.out.printf("%-12d %-25s %-15s%n", resultSet.getInt("driverId"),
							resultSet.getString("firstname") + " " + resultSet.getString("lastname"),
							resultSet.getString("nationality"));
				}
			}
	...
	public void constructorsNationality(int constructorId) {
		String sql = "select * from constructors c where c.constructorId = ?;";
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-20s %-30s %-15s%n", "Constructor ID", "Constructor Name", "Nationality");
				System.out.println("-------------------------------------------------------------------");

				while (resultSet.next()) {
					System.out.printf("%-20d %-30s %-15s%n", resultSet.getInt("constructorId"),
							resultSet.getString("name"), resultSet.getString("nationality"));
				}
			}
			System.out.println();
	...
	public void driversSeasonalLapTimeAvg(int driverId, int year) {
		String sql = "select d.firstname, d.lastname, r.year, dateadd(millisecond, avg(datediff(millisecond, 0, l.time)), 0) as avgTime from laps l join races r on r.raceId = l.raceId join drivers d on d.driverId = l.driverId where r.year = ? and l.driverId = ? group by d.firstname, d.lastname, r.year;";
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				System.out.printf("%-25s %-9s %-12s%n", "Name", "Season", "Avg Time");
				System.out.println("-----------------------------------------------");

				while (resultSet.next()) {
					System.out.printf("%-25s %-9s %-12s%n",
							resultSet.getString("firstname") + " " + resultSet.getString("lastname"),
							resultSet.getInt("year"), resultSet.getTime("avgTime"));
				}
			}
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				ResultSet rs2 = secondStatement.executeQuery(
						"select d.driverId, d.firstname, d.lastname from drivers d join driverRaces dr on dr.driverId = d.driverId where dr.raceId = "
								+ raceId + " and dr.finalPosition = -1");

				System.out.printf("%-12s %-25s%n", "Driver ID", "Name");
				System.out.println("--------------------------------");
				while (rs2.next()) {
					System.out.printf("%-12d %-25s%n", rs2.getInt("driverId"),
	...
	public void driversWinCount(int driverId) {
		...
		try {
			...
			if (!resultSet.isBeforeFirst()) {
				...
				PreparedStatement secondStatement = connection.prepareStatement(
						"select dc.year from driverChampionships dc where dc.driverId = ? and dc.winner = 1;");

				secondStatement.setInt(1, driverId);

				ResultSet resultSet2 = secondStatement.executeQuery();

				System.out.printf("%-8s%n", "Seasons");
				System.out.println("---------");
				while (resultSet2.next()) {
					System.out.printf("%-8s%n", resultSet2.getInt("year"));
				}
			}
			System.out.println();
